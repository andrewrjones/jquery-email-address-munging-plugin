...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    jade: {
      compile: {
        files: {
          "dist/index.html": ["docs/index.jade"]
        },
        options: {
          client: false
        }
      ...
      all: {
        options: {
          urls: ['1.9.1', '2.0.0b1'].map(function (version) {
            return 'http://localhost:<%= connect.server.port %>/test/jquery.emailaddressmunging.html?jquery=' + version;
          })
        }
      }
    },
    ...
    compress: {
      zip: {
        options: {
          ...
          archive: "jquery-email-address-munging-plugin-<%= pkg.version %>.zip"
        },
        files: [{
          expand: true,
          flatten: true,
